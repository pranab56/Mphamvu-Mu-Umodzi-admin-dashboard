...
import { useGetEventRequestQuery, useLazyGetSingleEventRequestQuery } from "@/features/request/requestApi";
...
export default function UrgentRequestsPage() {
  ...
  const { data: reportsData, isLoading } = useGetEventRequestQuery({ page: 1 });
  const [getSingleReport] = useLazyGetSingleEventRequestQuery();
  ...
  return (
    <div className="pb-20">
      {/* Header */}
      <div className="flex items-start gap-4 mb-4 md:mb-10">
        <div className='flex flex-col sm:flex-row sm:items-center justify-between w-full gap-2'>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-medium text-gray-900 tracking-tight leading-none">Urgent Event Request List</h1>
      ...
      {/* Main Content Card */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_10px_40px_rgb(0,0,0,0.01)] p-4 md:p-8 space-y-4 md:space-y-8">
        {/* Search Bar and Download Buttons */}
        <div className="flex flex-col lg:flex-row gap-4 items-center">
          <div className="relative w-full">
            <Input
              placeholder="Search here..."
              className="h-12 md:h-14 pl-12 rounded-xl bg-neutral-100 border-none text-sm placeholder:text-gray-400 focus-visible:ring-0"
              value={searchTerm}
