...
import { useDeleteEventMutation, useGetAllEventsQuery } from "@/features/event/eventApi";
import { format } from "date-fns";
import { CalendarX, ChevronLeft, ChevronRight, Loader2, MoreVertical, Search } from "lucide-react";
          ...
          <table className="w-full text-left">
            <thead className="bg-[#E5D5C9]/30 text-gray-500 font-normal text-sm">
              <tr>
                ...
                {type === "active" && <th className="px-6 py-4 font-normal text-center">Target</th>}
                <th className="px-6 py-4 font-normal text-center">Amount Raised</th>
          ...
          <div className="flex items-center gap-2">
            ...
            {Array.from({ length: meta.totalPage }, (_, i) => i + 1).map((p) => (
              <Button
                ...
                variant="ghost"
                onClick={() => setPage(p)}
                className={`h-10 w-10 p-0 rounded-full text-base transition-all ${p === page ? "bg-[#8B2F0E] text-white hover:bg-[#8B2F0E] shadow-sm transform scale-110" : "text-gray-400 hover:bg-gray-100"
                  }`}
              >
                {p}
              </Button>
